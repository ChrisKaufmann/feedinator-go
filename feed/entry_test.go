...
func TestEntry_Save(t *testing.T) {
	...
	ne := ge()
	if ne.Title != "newtitle" {
		t.Errorf("title != newtitle: %s", ne.Title)
	}
	if ne.Link != "newlink" {
		t.Errorf("link != newlink: %s", ne.Link)
	}
	if ne.Marked != "1" {
		t.Errorf("marked != 1: %s", ne.Marked)
	}
	if ne.FeedID != 2 {
		t.Errorf("FeedID != 2: %v", ne.FeedID)
	}
	if ne.Content != "newcontent" {
		t.Errorf("Content != newcontent: %s", ne.Content)
	}
	if ne.Unread != false {
		t.Errorf("Unread != false: %t", ne.Unread)
	}
}
...
func TestAllMarkedEntries(t *testing.T) {
	print("\tAllMarkedEntries()\n")
	e := ef()
	print("\t\tInitial\n")
	if len(AllMarkedEntries("test")) != 0 {
		t.Errorf("Len(allmarkedentries) != 0: %v", len(AllMarkedEntries("test")))
	}
	print("\t\tMarking\n")
	err := e.Mark()
	if err != nil {
	...
	if len(AllMarkedEntries("test")) != 1 {
